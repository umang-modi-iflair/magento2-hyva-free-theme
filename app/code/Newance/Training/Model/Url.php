...
class Url
{
    ...
    const CONTROLLER_SUBSCRIBER = 'subscriber';
    ...
    {
        if ($controllerName) {
            if ($controllerName == self::CONTROLLER_SUBSCRIBER) {
    ...
    public function getUrlPath($identifier, $controllerName)
    {
        ...
        switch ($this->getPermalinkType()) {
            ...
            case self::PERMALINK_TYPE_SHORT:
                if ($controllerName == self::CONTROLLER_SEARCH || $controllerName == self::CONTROLLER_SUBSCRIBER) {
                    return $this->getRoute() . '/' . $this->getRoute($controllerName) . '/' . $identifier;
                } else {
    ...
    /**
     * Retrieve training permalink config value
     * @param  string $key
    ...
    protected function _getConfig($key)
    {
        return $this->_scopeConfig->getValue(
            'newance_training/permalink/' . $key,
