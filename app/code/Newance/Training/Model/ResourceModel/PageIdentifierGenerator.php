...
class PageIdentifierGenerator
{
    ...
    public function generate(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        while (true) {
            ...
            $brandId = $brand->checkIdentifier($finalIdentifier, $object->getStoreId());

            if (!$postId && !$categoryId && !$brandId) {
                break;
            } else {
                if ($postId
                    && $postId == $object->getId()
                    && $object instanceof \Newance\Training\Model\Post
                ) {
                    break;
                }
                ...
                if ($categoryId
                    ...
                    && $object instanceof \Newance\Training\Model\Category
                ) {
                    break;
                }

                if ($brandId
                    && $brandId == $object->getId()
                    && $object instanceof \Newance\Training\Model\Brand
                ) {
                    break;
                }
            }
