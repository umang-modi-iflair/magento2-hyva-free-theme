...
/**
 * Training category and brand resource model
 */
class Post extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $this->_init('newance_training_post', 'post_id');
    }
    ...
    protected function _beforeDelete(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $this->getConnection()->delete($this->getTable('newance_training_post_store'), $condition);
        $this->getConnection()->delete($this->getTable('newance_training_post_category'), $condition);
        $this->getConnection()->delete($this->getTable('newance_training_post_brand'), $condition);
        $this->getConnection()->delete($this->getTable('newance_training_post_relatedproduct'), $condition);

        $this->getConnection()->delete($this->getTable('newance_training_post_relatedpost'), $condition);
        $this->getConnection()->delete(
            $this->getTable('newance_training_post_relatedpost'),
    ...
    /**
     * Assign post to store views, categories, brands, related posts, etc.
     *
     * @param \Magento\Framework\Model\AbstractModel $object
     * @return $this
     */
    protected function _afterSave(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $this->_updateLinks($object, $newIds, $oldIds, 'newance_training_post_store', 'store_id');
        ...
        if (is_array($newIds)) {
            ...
            $this->_updateLinks($object, $newIds, $oldIds, 'newance_training_post_category', 'category_id');
        }

        $newIds = (array)$object->getBrands();
        foreach ($newIds as $key => $id) {
            if (!$id) { // e.g.: zero
                unset($newIds[$key]);
            }
        ...
        if (is_array($newIds)) {
            $oldIds = $this->lookupBrandIds($object->getId());
            $this->_updateLinks($object, $newIds, $oldIds, 'newance_training_post_brand', 'brand_id');
        }

        $linksData = $object->getData('relatedposts_links');
        if (is_array($linksData)) {
            $oldIds = $this->lookupRelatedPostIds($object->getId());
            $this->_updateLinks($object, array_keys($linksData), $oldIds, 'newance_training_post_relatedpost', 'related_id', $linksData);
        }

        $linksData = $object->getData('relatedproducts_links');
        if (is_array($linksData)) {
            $oldIds = $this->lookupRelatedProductIds($object->getId());
            $this->_updateLinks($object, array_keys($linksData), $oldIds, 'newance_training_post_relatedproduct', 'related_id', $linksData);
        }
    ...
    protected function _afterLoad(\Magento\Framework\Model\AbstractModel $object)
    {
        if ($object->getId()) {
            ...
            $brands = $this->lookupBrandIds($object->getId());
            $object->setBrands($brands);
        }
    ...
    protected function _getLoadByIdentifierSelect($identifier, $storeIds, $isActive = null)
    {
        $select = $this->getConnection()->select()->from(
            ...
            ['cps' => $this->getTable('newance_training_post_store')],
    ...
    {
        return $this->_lookupIds($postId, 'newance_training_post_store', 'store_id');
    }
    ...
    {
        return $this->_lookupIds($postId, 'newance_training_post_category', 'category_id');
    }

    /**
     * Get brand ids to which specified item is assigned
     *
     * @param int $pageId
     * @return array
     */
    public function lookupBrandIds($postId)
    {
        return $this->_lookupIds($postId, 'newance_training_post_brand', 'brand_id');
    }
    ...
    {
        return $this->_lookupIds($postId, 'newance_training_post_relatedpost', 'related_id');
    }
    ...
    {
        return $this->_lookupIds($postId, 'newance_training_post_relatedproduct', 'related_id');
    }
